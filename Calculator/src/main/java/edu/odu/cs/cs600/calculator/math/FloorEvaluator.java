...
public class FloorEvaluator implements IUnaryEvaluator {
	 ...
	 * into a range; bracketing value by two binary values where 2^k <= value < 2^(k+1).
	 * Perform a binary search for value within this range.  Method handles special cases
	 * such as 0.0 >= value < 1.0 and -1.0 >= value < 0.0 as these can't be encapsulated by
	 * the above process.  Negative input is passed to {@link MathUtil.ceiling} under the
	 * realization that floor(a) = -(ceiling(-a)). 
	 * @param value
	 * @return
